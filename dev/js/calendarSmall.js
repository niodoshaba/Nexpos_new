    ...
    var Calendar = {
      ...
      dateClicked: function () {
        ...
        for (var i = 0; i < this.activeDates.length; i++) {
          ...
          let calendarPickDate = document.getElementsByClassName('calendarPickDate')
          let S_calendarAll = document.getElementsByClassName('S_calendarAll')
  
  
          if(calendarPickDate.length == 1){
          calendarPickDate[0].addEventListener('click',function(){
            calendarPickDate[0].value = ""
            S_calendarAll[0].style.display="block"
          })
        }else{
          
            calendarPickDate[0].addEventListener('click',function(){
              calendarPickDate[0].value = ""
              calendarPickDate[1].value = ""
              S_calendarAll[0].style.display="block"
            }) 
          calendarPickDate[1].addEventListener('click',function(){
            calendarPickDate[0].value = ""
            calendarPickDate[1].value = ""
            S_calendarAll[0].style.display="block"
          })
          ...
          this.activeDates[i].addEventListener('click', function (event) {
            ...
            // console.log(DAILY_RES[0].DAILY_DATE)
              ...
              // console.log(this.parentNode.parentNode.parentNode.firstElementChild)
              // this.parentNode.parentNode.parentNode
              // this.parentNode.parentNode.parentNode.firstElementChild.value=resAbleDate;
              
              if(calendarPickDate.length==2){
  
                  if(calendarPickDate[0].value != ""){
                    calendarPickDate[1].value = resAbleDate
        
                    this.classList.add('calendarDate--selected')
                  }else{
                    calendarPickDate[0].value = resAbleDate
                    _this.removeActiveClass()
                    this.classList.add('calendarDate--selected')
                  }
  
                  if(calendarPickDate[0].value != "" && calendarPickDate[1].value != ""){
                    S_calendarAll[0].style.display="none"
                  }
              }else{
              calendarPickDate[0].value = resAbleDate
              _this.removeActiveClass()
