  ...
  $(".addBtn").click(function () {
    ...
    $(".title").after(`
      <tr class="input">
        ...
        <td><input class="empPsw" type="text" size="15" style="width:150px;"></td>
  ...
  function edit() {
    $(".edit").click(function () {
      ...
      tr.find("td:eq(1)").append(
        `<input class="empTitle" value="${empTitle}" type="text" size="10">`
      );
      // 名稱
      let empName = tr.find("td:eq(2)").text();
      tr.find("td:eq(2)").text("");
      tr.find("td:eq(2)").append(
        `<input class="empName" value="${empName}" type="text" size="15">`
      );
      // 手機
      let empPhone = tr.find("td:eq(3)").text();
      tr.find("td:eq(3)").text("");
      tr.find("td:eq(3)").append(
        `<input class="empPhone" value="${empPhone}" type="text" size="15">`
      );
      // 密碼
      let empPsw = tr.find("td:eq(4)").text();
      tr.find("td:eq(4)").text("");
      tr.find("td:eq(4)").append(
        `<input class="empPsw" value="${empPsw}" type="text" size="15" style="width:150px;">`
      );
      // 狀態
      let empStatus = tr.find("td:eq(5)").text();
      tr.find("td:eq(5)").text("");
      tr.find("td:eq(5)").append(
        `<select class="empStatus"><option value="1">在職</option><option value="0">離職</option></select>`
      );
      // 編輯
